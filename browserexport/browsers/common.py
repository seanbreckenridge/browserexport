...
@dataclass
class Browser:
    query: str  # query to extract visit from database
    ...
    def detect(cls, path: PathIshOrConn) -> bool:
        """
        Run the detector against the given path/connection to detect the database type
        """
        query = f"SELECT * FROM {cls.detector}"
        try:
            list(_execute_query(path, query))
            return True
        except sqlite3.OperationalError as sql_err:
            logger.debug(f"Detector query for {cls.__name__} failed: {sql_err}")
            return False
